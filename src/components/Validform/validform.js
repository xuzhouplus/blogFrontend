...
export const showValidformError = (obj, msg) => {
	const inputParent = obj.closest('.form-group');
	inputParent.find('.Validform_checktip').addClass('Validform_wrong').text(msg);
	inputParent.find('.info').css({
		'margin-top': 0
	}).show().animate({
		'margin-top': '10px'
	}, 200);
...
export const hideValidformError = (obj) => {
	const inputParent = obj.closest('.form-group');
	inputParent.find('.Validform_checktip').removeClass('Validform_wrong').empty();
	inputParent.find('.info').hide();
};

export const initValidform = (formObj, options) => {
	const defaultOptions = {
		...
		tiptype: function (msg, o, cssctl) {
			if (!o.obj.is('form')) {//验证表单元素时o.obj为该表单元素，全部验证通过提交表单时o.obj为该表单对象;
				let objTip = o.obj.closest('.form-group').find('.Validform_checktip');
				cssctl(objTip, o.type);
				objTip.text(msg);
				let infoObj = o.obj.closest('.form-group').find('.info');
				if (o.type === 2) {
					infoObj.fadeOut(200);
				} else {
	...
	return formObj.Validform(options);
};
